...
class ImagePlane(pgl.GLMeshItem):
    def __init__(self, image_path, size=10.0, pos=(0, 0, 0)):
        ...
        meshdata = pgl.MeshData(vertexes=verts, faces=faces, texcoords=texcoords)
...
class My3DViewer(QWidget):
    def __init__(self, parent=None):
        ...
        # إنشاء صورة ملونة شفافة نصفية
        file_of_image = os.path.join(os.getcwd(), "image.png")
        create_image(path_of_file=file_of_image, width=100, height=100, color=(0, 128, 255, 200))
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("3D Viewer with Correct Texture Mapping")
...
if __name__ == "__main__":
    ...
    window = MainWindow()
    window.show()
    sys.exit(app.exec_())            image.putpixel((x, y), color)
