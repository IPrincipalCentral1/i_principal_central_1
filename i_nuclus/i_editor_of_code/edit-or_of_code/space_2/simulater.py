...
class GLBoxWidget(QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.camera_angle_x = 0.0  # للكاميرا - أعلى / أسفل
        self.camera_angle_y = 0.0  # للكاميرا - يمين / يسار (اختياري)
        

        self.zoom = -10.0            # موضع الكاميرا على محور Z
        self.camera_x = 0.0          # تحريك الكاميرا أفقياً
        self.camera_y = 0.0          # تحريك الكاميرا عمودياً
        self.camera_angle_y = 0.0    # دوران الكاميرا حول المحور Y
        
        self.angle_x = 0.0           # دوران المجسم حول X
        self.angle_y = 0.0           # دوران المجسم حول Y
        ...
        self.scene_pos_x = 0.0
        
        self.scene_pos_y = 0.0
    
    
        self.scene_pos_z = 0.0
    
    
        
        self.scene_angle_x = 0.0


        self.scene_angle_y = 0.0
    ...
    def get_camera_right(self):
        # احسب اتجاه اليمين بناءً على دوران الكاميرا حول المحور Y فقط
        yaw = math.radians(self.camera_angle_y)
        return (math.cos(yaw), 0, -math.sin(yaw))  # right vector
    
    def get_camera_up(self):
        # اتجاه أعلى ثابت غالبًا في هذه الحالة
        return (0, 1, 0)
    ...
    def get_camera_direction(self):
        # زاوية بالدرجات → راديان
        yaw = math.radians(self.camera_angle_y)
        pitch = math.radians(self.camera_angle_x)
    
        # اتجاه الكاميرا (forward vector)
    ...
    def get_camera_right(self):
        # فقط Yaw مهم للاتجاه الأفقي لليمين
        yaw = math.radians(self.camera_angle_y)
    ...
    def paintGL(self):
        ...
        # دوران الكاميرا (يؤثر على النظام كله)
        glRotatef(self.camera_angle_x, 1.0, 0.0, 0.0)
        glRotatef(self.camera_angle_y, 0.0, 1.0, 0.0)
    
        # تحريك كامل المحاكاة (بما فيها المجسم) حسب موقع الكاميرا الظاهري
        glTranslatef(self.scene_pos_x, self.scene_pos_y, self.scene_pos_z)
    
        # تدوير المجسم إن أردت
    ...
    #def paintGL(self):
        ...
        ## تحريك المشهد في الفضاء حسب الإزاحة 3D
        #glTranslatef(self.scene_pos_x, self.scene_pos_y, self.scene_pos_z)
    ...
    #def paintGL(self):
        ...
        ## تحريك الكاميرا (المشهد المعاكس)

        #glTranslatef(self.camera_x, self.camera_y, self.zoom)

        ## تدوير الكاميرا (نظرة المشاهد)
        #glRotatef(self.camera_angle_x, 1.0, 0.0, 0.0)
        #glRotatef(self.camera_angle_y, 0.0, 1.0, 0.0)



        #glTranslatef(self.pos_x, self.pos_y, -10)

        ## دوران المجسم (إن أردت)
        #glRotatef(self.angle_x, 1.0, 0.0, 0.0)
        #glRotatef(self.angle_y, 0.0, 1.0, 0.0)
    ...
    #def paintGL(self):
        ...
        ## تحريك المشهد بالكامل بناء على الإزاحة (scene position)
        #glTranslatef(self.scene_pos_x, self.scene_pos_y, self.zoom)

        ### إذا عندك تدوير للمشهد (مثلاً):
        ##glRotatef(self.scene_angle_x, 1.0, 0.0, 0.0)
        ##glRotatef(self.scene_angle_y, 0.0, 1.0, 0.0)
    ...
    #def paintGL(self):
        ...
        ## تحريك الكاميرا في الفضاء

        #glTranslatef(self.camera_x, self.camera_y, self.zoom)

        ## دوران الكاميرا حول المحور Y (يمين / يسار)

        #glRotatef(self.camera_angle_y, 0.0, 1.0, 0.0)

        ## دوران المجسم (اختياري)

        #glRotatef(self.angle_x, 1.0, 0.0, 0.0)
        #glRotatef(self.angle_y, 0.0, 1.0, 0.0)

        ## رسم المكعب أو المجسم الملمس
    ...
    #def paintGL(self):
        ...
        ## دوران الكاميرا حول المحورين Y و X (يمين/يسار - أعلى/أسفل)
    ...
    def keyPressEvent(self, event):
        ...
        if modifiers == Qt.ControlModifier:
            right = self.get_camera_right()
            up = self.get_camera_up()
    
            if key == Qt.Key_Right:
                self.scene_pos_x -= right[0] * move_speed
                self.scene_pos_y -= right[1] * move_speed
                self.scene_pos_z -= right[2] * move_speed
            elif key == Qt.Key_Left:
                self.scene_pos_x += right[0] * move_speed
                self.scene_pos_y += right[1] * move_speed
                self.scene_pos_z += right[2] * move_speed
            elif key == Qt.Key_Up:
                self.scene_pos_x -= up[0] * move_speed
                self.scene_pos_y -= up[1] * move_speed
                self.scene_pos_z -= up[2] * move_speed
    ...
    #def keyPressEvent(self, event):
        ...
        #move_speed = 0.5  # مقدار الحركة



        #if modifiers == Qt.ShiftModifier:
            #if key == Qt.Key_Up:
                #self.camera_angle_x -= 5.0  # تدوير الكاميرا لأعلى
            #elif key == Qt.Key_Down:
                #self.camera_angle_x += 5.0  # تدوير الكاميرا لأسفل
            #elif key == Qt.Key_Right:
                #self.camera_angle_y += 5  # دوران لليمين
                ##self.update()
            #elif key == Qt.Key_Left:
                #self.camera_angle_y -= 5  # دوران لليسار
            #self.update()
            ...
            #right = self.get_camera_right()
            #up = (0, 1, 0)  # ثابت في العادة

            #if key == Qt.Key_Left:
                ## تحرك المشهد يميناً بالنسبة للكاميرا يعني نقل عكس اتجاه اليمين
                #self.scene_pos_x -= right[0] * move_speed
                #self.scene_pos_y -= right[1] * move_speed  # غالباً 0
                #self.scene_pos_z -= right[2] * move_speed
            ...
            #elif key == Qt.Key_Up:
                ## تحرك المشهد للأعلى بالنسبة للكاميرا (المحور y)
                #self.scene_pos_x += up[0] * move_speed
                #self.scene_pos_y += up[1] * move_speed
                #self.scene_pos_z += up[2] * move_speed
        ...
        ##if modifiers == Qt.ControlModifier:
            ##if key == Qt.Key_Up:
                ##self.scene_pos_y += 0.5   # تحرك المشهد للأعلى
            ##elif key == Qt.Key_Down:
                ##self.scene_pos_y -= 0.5   # تحرك المشهد للأسفل
            ##elif key == Qt.Key_Left:
                ##self.scene_pos_x -= 0.5   # تحرك المشهد لليسار
            ##elif key == Qt.Key_Right:
                ##self.scene_pos_x += 0.5   # تحرك المشهد لليمين
            ##self.update()
            ##return
        ...
        ##elif modifiers == Qt.ControlModifier:
            ##if key == Qt.Key_Up:
                ##self.camera_y += 0.5  # الكاميرا تصعد للأعلى
            ##elif key == Qt.Key_Down:
                ##self.camera_y -= 0.5  # الكاميرا تنزل للأسفل
            ##elif key == Qt.Key_Left:
                ##self.camera_x -= 0.5
            ##elif key == Qt.Key_Right:
                ##self.camera_x += 0.5
