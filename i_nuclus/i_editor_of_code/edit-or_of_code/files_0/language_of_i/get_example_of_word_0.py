...
for synset in wn.all_synsets():
    ...
    print(f"word = {word} . sorted(list_0) = {sorted(list_0)}")
    
    
    counter_0 = 0
    
    while ((counter_0 < len(word_with_definition)) and (word_with_definition[counter_0][0] > word)):
    
        counter_0 += 1
    
    word_with_definition.insert(counter_0, [word, sorted(list_0)])

    
    #word_with_definition.append([word, sorted(list_0)])
    
    print(f"word_with_definition[counter_0] = {word_with_definition[counter_0]}")
    
        
    #word_with_definition[-1][1].extend(sorted(list_0))    
...
with open(file_0, "w") as f_:
    ...
    while (counter_0 < len(word_with_definition)):
        ...
        while (counter_1 < len(element_1)):
        
            
            f_.write(f"    {element_1[counter_1]}\n")
    
    
            print(f"counter_1 = {counter_1} . element[0] = {element[0]} . element_1[counter_1] = {element_1[counter_1]}")
