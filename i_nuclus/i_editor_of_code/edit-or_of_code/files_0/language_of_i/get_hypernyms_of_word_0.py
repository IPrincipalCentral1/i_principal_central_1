...
def show_relations(list_of_word, word):
    ...
    for syn in synsets:
        ...
        list_of_word.append(["Hypernyms", [s.name() for s in syn.hypernyms()]])
        
        print(f"  🔷 (الترادفات) Hyponyms: {[s.name() for s in syn.hyponyms()]}")
        
        list_of_word.append(["Hyponyms", [s.name() for s in syn.hyponyms()]])
        
        
        print(f"  🔷 (أسماء مفرطة للمثيلات) Instance Hypernyms: {[s.name() for s in syn.instance_hypernyms()]}")


        list_of_word.append(["Instance Hypernyms", [s.name() for s in syn.instance_hypernyms()]])

        print(f"  🔷 (أمثلة على المرادفات) Instance Hyponyms: {[s.name() for s in syn.instance_hyponyms()]}")


        list_of_word.append(["Instance Hyponyms", [s.name() for s in syn.instance_hyponyms()]])

        print(f"  🔷 (أسماء الأعضاء المتجانسة) Member Holonyms: {[s.name() for s in syn.member_holonyms()]}")
 
 
        list_of_word.append(["Member Holonyms", [s.name() for s in syn.member_holonyms()]])
 
        print(f"  🔷 (جزء من الكلمات المتجانسة) Part Holonyms: {[s.name() for s in syn.part_holonyms()]}")


        list_of_word.append(["Part Holonyms", [s.name() for s in syn.part_holonyms()]])

        print(f"  🔷 (مرادفات المادة) Substance Holonyms: {[s.name() for s in syn.substance_holonyms()]}")



        list_of_word.append(["Substance Holonyms", [s.name() for s in syn.substance_holonyms()]])

        print(f"  🔷 (أسماء الأعضاء) Member Meronyms: {[s.name() for s in syn.member_meronyms()]}")



        list_of_word.append(["Member Meronyms", [s.name() for s in syn.member_meronyms()]])

        print(f"  🔷 (مرادفات الأجزاء) Part Meronyms: {[s.name() for s in syn.part_meronyms()]}")



        list_of_word.append(["Part Meronyms", [s.name() for s in syn.part_meronyms()]])


        print(f"  🔷 (مرادفات المواد) Substance Meronyms: {[s.name() for s in syn.substance_meronyms()]}")
 
 
        list_of_word.append(["Substance Meronyms", [s.name() for s in syn.substance_meronyms()]])
        ...
        list_of_word.append(["Also See", [s.name() for s in syn.also_sees()]])
        ...
        list_of_word.append(["Similar To", [s.name() for s in syn.similar_tos()]])
