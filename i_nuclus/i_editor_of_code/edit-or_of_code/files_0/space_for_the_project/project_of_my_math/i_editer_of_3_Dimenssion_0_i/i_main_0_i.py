...
k_damp = 1.0          # معامل التخميد (احتكاك)
...
def update_physics():
    ...
    # ضغط السائل المتبادل (قانون باسكال + حفظ الحجم)
    # A_small*y_small + A_big*y_big = ثابت  -> مشتقة زمنية: A_small*v_small + A_big*v_big = 0
    v_big = -(A_small / A_big) * v_small

    # القوة المؤثرة على المكبس الصغير
    F_small = -m_small * g - rho * g * (y_small - y_big) - k_damp * v_small
    # القوة على المكبس الكبير (معاكسة للحفاظ على الضغط)
    F_big = -m_big * g + rho * g * (y_small - y_big) - k_damp * v_big
...
# المكبسان (دوائر تمثل كل مكبس)
piston_small, = ax.plot([], [], 'ro', markersize=20)
piston_big, = ax.plot([], [], 'bo', markersize=30)
