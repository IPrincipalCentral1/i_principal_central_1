...
ax.invert_yaxis()  # اجعل الشكل U طبيعي
...
def animate(frame):
    ...
    delta_h, a_small, a_big = update_physics()

    # طباعة القيم في الطرفية
    if frame_counter % 5 == 0:  # اطبع كل 5 إطارات لتقليل الزحمة
        print(f"Frame {frame_counter:4d} | "
              f"y_small={y_small:+.4f} m | v_small={v_small:+.4f} m/s | "
              f"y_big={y_big:+.4f} m | v_big={v_big:+.4f} m/s | "
              f"Δh={delta_h:+.4f} m")
