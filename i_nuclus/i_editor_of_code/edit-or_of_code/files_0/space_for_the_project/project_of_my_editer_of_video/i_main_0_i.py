...
class _____name_of_class_0_____(Scene):
    ...
    def construct(self):
        ...
        # إعداد البندولات: (الموضع الأفقي, لون1, زاوية1°, طول1, لون2, زاوية2°, طول2)
        pendulum_settings = [
            (-4, RED, 30, 2.5, BLUE, -10, 1.5),
            (-2, GREEN, -20, 2.2, YELLOW, 15, 1.3),
            (0, PURPLE, 45, 2.4, ORANGE, -5, 1.6),
            (2, TEAL, -10, 2.3, GOLD, 20, 1.5),
            (4, MAROON, 60, 2.1, PINK, -15, 1.4),
        ]
        ...
        for x_pos, color1, angle1, L1, color2, angle2, L2 in pendulum_settings:
            origin = np.array([x_pos, 2, 0])
        ...
        self.add(pendulum_pairs, trail_group)
        ...
        def update_double_pendulums(mob, dt):
            for i, pair in enumerate(mob):
                ...
                # إضافة أثر الحركة للبندول الثاني
                trail = trail_group[i]
                dot_trace = Dot(p2, radius=0.02, color=bob2.get_color())
                trail.add(dot_trace)
                if len(trail) > 150:
