...
class _____name_of_class_0_____(Scene):
    ...
    def construct(self):
        ...
        # إعداد كل بندول مزدوج: (لون1, زاوية1°, طول1, لون2, زاوية2°, طول2)
        pendulum_settings = [
        ...
        for color1, angle1, L1, color2, angle2, L2 in pendulum_settings:
            rod1 = Line(ORIGIN, L1 * DOWN, stroke_color=color1, stroke_width=6)
        ...
        # دالة التحديث — تستعمل dt من المشهد
        def update_double_pendulums(mob, dt):
            for i, pair in enumerate(mob):
                ...
                # تقريب فيزيائي للبندول المزدوج
                denom1 = L1 * (2 - np.cos(2 * δ))
                denom2 = L2 * (2 - np.cos(2 * δ))
                α1 = (-gravity * (2 * np.sin(θ1) + np.sin(θ1 - 2 * θ2))
                      - 2 * np.sin(δ) * (ω2**2 * L2 + ω1**2 * L1 * np.cos(δ))) / denom1
                α2 = (2 * np.sin(δ) * (ω1**2 * L1 * (2) + gravity * (2) * np.cos(θ1) + ω2**2 * L2 * np.cos(δ))) / denom2
                ...
                # مواضع النقاط
                origin = ORIGIN + np.array([i * 3, 0, 0])  # إزاحة كل زوج قليلاً
