...
class _____name_of_class_0_____(ThreeDScene):
    ...
    def construct(self):
        # إعداد الكاميرا لتُظهر العمق
        self.set_camera_orientation(phi=65 * DEGREES, theta=30 * DEGREES)
        
        # مربع في المستوى XY (Z = 0)
        sq = Square(side_length=2)
        sq.set_fill(YELLOW, opacity=0.6)
        sq.set_stroke(width=3)
        sq.move_to([0, 0, 1])   # تحريك في المحور z = 1

        # لجعله يظهر كمستوى مائل في الفضاء:
        sq.rotate(PI/6, axis=RIGHT)   # ميل حول محور X
        sq.rotate(PI/8, axis=UP)      # تدوير إضافي حول محور Y

        self.play(Create(sq))
        # تحريك الكاميرا حول المشهد لعرضه ثلاثي الأبعاد
...
def i_main_0_i():
    ...
    #os.system(f"{sys.executable} -m manimgl {i_name_of_file_0_i} {i_name_of_project_0_i}")
