...
def i_extract_sentences_from_html_0_i(i_path_of_input_0_i, i_path_of_output_0_i):
    ...
    """
    extract_sentences_from_html.py
    
    يأخذ ملف HTML، يستخرج النص المرئي، يقسمه إلى جمل، ويكتب الجمل "الطبيعية" في ملف نصي/HTML إخراج.
    تعريف "جملة طبيعية" هنا:
     - تحتوي على حروف (عربية أو لاتينية أو أحرف لغوية أخرى)
     - ليست مجرد أرقام/تواريخ/روابط/مقتطف كود
     - تحتوي على أكثر من كلمة واحدة (قابلة للتعديل)
    """
    ...
    MIN_WORDS = 2            # أقل عدد كلمات لجملة تُقبل
    MAX_SYMBOL_RATIO = 0.6   # إن كان نسبة الأحرف غير الحرفية أكبر من هذا => يعتبر غير طبيعياً
    MIN_LETTER_COUNT = 1     # على الأقل حرف واحد من الأبجديات (للتفرقة عن أرقام فقط)
    SPLIT_PATTERN = r'(?<=[\.\!\?\u061f\u061b\u060c\u06d4])\s+'  # نقط، علامات استفهام عربية، فاصلة عربية (تقسيم جمل)
    # يمكنك تعديل SPLIT_PATTERN لإضافة قواعد تقسيم أخرى
    
    # أنماط تستبعد الجمل التي تبدو روابط أو مسارات ملفات أو شفرات
    URL_PATTERN = re.compile(r'https?://|www\.\S+')
    FILEPATH_PATTERN = re.compile(r'(/[^\s]+){2,}|[A-Za-z]:\\')  # مسارات unix أو windows
    CODE_SIGNS = re.compile(r'<\/?\w+>|=\s*["\']|;\s*$')  # وجود شيفرة HTML / assignment / ; في نهاية السطر
    ONLY_SYMBOLS = re.compile(r'^[\W_]+$', re.UNICODE)
    
    # مجموعة أحرف أبجدية (عربية + لاتينية + كيريلية كمثال) للبحث عن وجود حروف
    LETTER_PATTERN = re.compile(r'[\p{L}]', re.UNICODE)
    
    
    # fallback للبحث عن أحرف (بعض بيئات بايثون لا تدعم \p{L} مباشرة)
    AR_LAT_LETTER = re.compile(r'[A-Za-z\u0600-\u06FF\u0750-\u077F\uFB50-\uFDFF\uFE70-\uFEFF]', re.UNICODE)
    
    
    def visible_text_from_html(html: str) -> str:
        """استخرج النص المرئي من الـ HTML مع إزالة script/style وـ comments."""
        soup = BeautifulSoup(html, "lxml")
    
        # ازالة السكربت والستايل ونصوص الـhead وأشياء غير مرئية
        for tag in soup(['script', 'style', 'noscript', 'iframe', 'header', 'footer', 'nav', 'meta', 'link']):
            tag.decompose()
    
        # الحصول على النص المتبقي
    ...
    def split_into_sentences(text: str) -> list:
        """قسّم النص إلى جمل باستخدام نمط بسيط (يمكن تحسينه لاحقًا)."""
        # أولاً نفصل عند أسطر جديدة ثم نستخدم نمط الفصل
        # استبدل الفواصل/النصوص التي لا تفصل جملًا بنقطة مؤقتة إذا رغبت
        parts = re.split(SPLIT_PATTERN, text)
        # بعض الأجزاء قد تحتوي على عدة جمل متلاصقة بدون نقط مع علامات أخرى، نحتفظ بها كما هي
        sentences = [p.strip() for p in parts if p.strip()]
        return sentences
    
    
    def looks_like_natural_language(sent: str) -> bool:
        """فلترة: هل هذه الجملة على الأغلب لغة طبيعية؟"""
        s = sent.strip()
    
        # لا تقبل الجمل الفارغة
        if not s:
            return False
    
        # إذا كانت تحتوي على رابط أو مسار ملف، استبعدها
        if URL_PATTERN.search(s) or FILEPATH_PATTERN.search(s):
            return False
        ...
        # إذا كانت كلها رموز/علامات
        if ONLY_SYMBOLS.match(s):
            return False
    
        # عدد كلمات كعامل بسيط
        words = re.findall(r'\w+', s, flags=re.UNICODE)
        if len(words) < MIN_WORDS:
            return False
    
        # وجود حروف (عربية/لاتينية/سواها) — نستخدم fallback AR_LAT_LETTER
        if AR_LAT_LETTER.search(s) is None:
            return False
    
        # نسبة الرموز غير الأبجدية (تقديرية)
        total_chars = len(s)
        non_alpha = len(re.findall(r'[^0-9A-Za-z\u0600-\u06FF\u0750-\u077F\uFB50-\uFDFF\uFE70-\uFEFF\s]', s))
        if total_chars > 0 and (non_alpha / total_chars) > MAX_SYMBOL_RATIO:
            return False
    
        # تمر كل الفلاتر => اعتبرها جملة طبيعية
        return True
    
    
    def extract_sentences_from_html(html_text: str) -> list:
        visible = visible_text_from_html(html_text)
        candidate_sentences = split_into_sentences(visible)
        natural_sentences = [s for s in candidate_sentences if looks_like_natural_language(s)]
        return natural_sentences
    
    
    def i_extract_sentences_from_html_1_i(input_path, output_path):
        
        
        #if len(sys.argv) < 3:
            #print("Usage: python extract_sentences_from_html.py input.html output.txt")
            #sys.exit(1)
        ...
        sentences = extract_sentences_from_html(html)
        # كتابة النتائج كملف نصي (واحد في كل سطر)
        with open(output_path, 'w', encoding='utf-8') as out:
            for s in sentences:
                out.write(s.strip() + '\n')
    
        print(f"Done. Extracted {len(sentences)} sentences -> {output_path}")
    
    
    i_extract_sentences_from_html_1_i(input_path=i_path_of_input_0_i, output_path=i_path_of_output_0_i)
...
def main():
    ...
    i_v_0_i = extract_list_2(text=text, separater_of_words=separater_of_words, not_counted_words=not_counted_words)
    ...
    i_extract_sentences_from_html_0_i(
                            
                            i_path_of_input_0_i=r"/mnt/mydisk/shared_directory_on_PC/my_link_updated/i_directory/folder_for_website_0/index.html", 
                            
                            i_path_of_output_0_i=r"i_result_of_sentences_text_0_i.txt"
