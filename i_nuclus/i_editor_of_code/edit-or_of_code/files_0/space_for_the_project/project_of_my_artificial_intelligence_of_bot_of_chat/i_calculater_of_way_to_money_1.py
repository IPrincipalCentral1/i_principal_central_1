













list_of_liberary_to_install = [

                            ["requests"] ,
                            
                            

]










import os



import sys

import subprocess

import platform

import traceback



counter_0 = 0


while (counter_0 < len(list_of_liberary_to_install)):

        
    try:
    
    
        print(f"\n\n\npip install {list_of_liberary_to_install[counter_0][0]}\n\n\n")
    
        #os.system(f"pip3 install {list_of_liberary_to_install[counter_0][0]}")
    
        
        
        subprocess.check_call([sys.executable, "-m", "pip", "install", f"{list_of_liberary_to_install[counter_0][0]}"])
    
    
            
    except:
    
            
                    
        traceback.print_exc()
        
        error = traceback.format_exc()
        
        semaphore = True
        
        print(f"Erreur : {str(error)}")
        
    
    
    counter_0 += 1
    



import requests

import time

from pathlib import Path




cwd = os.path.dirname(os.path.abspath(__file__))



import i_math_i_am_i_0


number_of_digit_after_the_floating_point = _____number_of_digit_after_the_floating_point_____





  


number_of_calcule = _____number_of_calcule_____


supported_currencies = _____supported_currencies_____



list_of_result = _____list_of_result_____



number_of_chunk_0 = "___number_of_chunk_0___"





latest_type_of_int = "_____latest_type_of_int_____"



the_length_of_1_complete_number_of_your_int = "_____the_length_of_1_complete_number_of_your_int_____"








#m = i_math_i_am_i_0.calculatrice_2(s=oper, l_=[], n=0, number_of_digit_after_the_floating_point_=number_of_digit_after_the_floating_point, e=False)


def wait(time_in_number):

    t1 = time.time()
    
    
    t2 = time.time()    
   

    while (t2 - t1 < time_in_number):

        t2 = time.time()    



def wait_for_the_next_step_0():


    message = "this file should contain just 'true' for the mixer to make the next step"

    file_0 = os.path.join(cwd, "i_run_mixer_1.txt")

    with open(file_0, "r") as f_:
    
        content = f_.read(os.path.getsize(file_0))
    
    
    counter_0 = 0
    
    
    while (content != message):
    
                
        with open(file_0, "r") as f_:
        
            content = f_.read(os.path.getsize(file_0))
        

        #counter_0 += 1
        
        

def wait_for_the_next_step_1():


    message = "this file should contain just 'true' for the mixer to affect the step"

    file_0 = os.path.join(cwd, "i_run_mixer_2.txt")

    with open(file_0, "r") as f_:
    
        content = f_.read(os.path.getsize(file_0))
    
    
    counter_0 = 0
    
    
    while (content != message):
    
                
        with open(file_0, "r") as f_:
        
            content = f_.read(os.path.getsize(file_0))


        #counter_0 += 1



def get_paypal_like_rate(from_currency, to_currency, paypal_margin=0.05):



    url = f"https://open.er-api.com/v6/latest/{from_currency}"


    response = requests.get(url)

    print("Status:", response.status_code)

    data = response.json()


    if "rates" not in data or to_currency not in data["rates"]:
        raise Exception("فشل في الحصول على سعر الصرف.")


    real_rate = data["rates"][to_currency]

    paypal_rate = real_rate * (1 - paypal_margin)

    extract_ed = real_rate * paypal_margin

    return real_rate, extract_ed, paypal_rate

# تجربة





def mixer_0(list_0, file):

    list_1 = [0, 0]


    #file_0 = os.path.join(cwd, "currency_0.csv")

    with open(file, "w") as f_:
    
        
    
        counter_0 = 0
        
        while (counter_0 < len(list_0)):
        
            
            counter_1 = 0
            
            while (counter_1 < len(list_0)):
                
                
                            
                try:
                
                
                    real, extract_ed, paypal_like = get_paypal_like_rate(list_0[counter_0], list_0[counter_1])
                
                    print(f" 1 {list_0[counter_0]} = {real:.2f} {list_0[counter_1]} (PayPal ≈ {paypal_like:.2f} {list_0[counter_1]})")
                
                    f_.write(f"1;{list_0[counter_0]};{real:.2f};{list_0[counter_1]};bank;{paypal_like:.2f};{list_0[counter_1]};extract-ed;{extract_ed:.2f}\n")
                
                
                except Exception as e:
                
                
                    print(f"❌ فشل التحويل إلى : {e}")
                
                
                
                counter_1 += 1
            
            
            counter_0 += 1
        
    
    
def get_from_file(file):


    list_of_result = []

    with open(file, "r") as f_:
    
        content = f_.read(os.path.getsize(file))
    
    
    v_0 = content.split("\n")
    
    counter_0 = 0
    
    while (counter_0 < len(v_0)):
    
    
        if (v_0[counter_0] != ""):
        
            v_2 = v_0[counter_0].split(";")
        
            s_0 = v_2[5]
            
            v_1 = s_0.split(".")
            
            n_0 = int(v_1[0]) + (int(v_1[1]) / (10 ** 2))
            
            list_of_result.append([v_2[1], n_0, v_2[3]])
            
        counter_0 += 1

    return list_of_result


def finder_0(list_0, element):

    counter_0 = 0
    
    while ((counter_0 < len(list_0)) and (list_0[counter_0][0] != element)):
    
        counter_0 += 1

    
    return counter_0


def finder_1(list_0, element_0, element_1):

    counter_0 = 0
    
    while ((counter_0 < len(list_0)) and (list_0[counter_0][0] != element_0)):
    
        counter_0 += 1

    if (counter_0 < len(list_0)):
    
                
        while ((counter_0 < len(list_0)) and (list_0[counter_0][2] != element_1)):
        
            counter_0 += 1
            
    else:
    
        counter_0 = len(list_0)
        
        
    
    return counter_0




def transformer_0(list_0, unity_0, unity_1, amount):

    counter_0 = finder_1(list_0=list_0, element_0=unity_0, element_1=unity_1)
    
    semaphore_of_error = False

    amount_of_result = 0.0

    if ((counter_0 < len(list_0))):

        
        operation = f"{amount} * {list_0[counter_0][1]}"
        
        
        m = i_math_i_am_i_0.calculatrice_2(s=operation, l_=[], n=0, number_of_digit_after_the_floating_point_=number_of_digit_after_the_floating_point, e=False)
        
        if (m[0] != True):
        
            amount_of_result = m[1][0]
        
        
        
    else:
    
        semaphore_of_error = True
        
    
    return  [semaphore_of_error, amount_of_result]




def transform_and_calculate_0(list_0, amount):
    
    
    
        
    
    list_of_unity = list_0
    
    
    
    result_1 = [False, amount]
    
    
    #print(f"\n\n list_of_unity = {list_of_unity} \n\n result_1 = {result_1} \n\n")
    
    
    counter_0 = 0
    
    
    while (counter_0 + 1 < len(list_of_unity)):
    
        
        result_1 = transformer_0(list_0=list_of_result, unity_0=list_of_unity[counter_0], unity_1=list_of_unity[counter_0 + 1], amount=result_1[1])
        
        counter_0 += 1
    
    
    
    #print(f"\n\n\n new : result_1 = {result_1} \n\n\n")
    
    
    return result_1
    







def int_to_str_0(number_0):

    str_0 = str(number_0)
    
    counter_0 = len(str_0)
    


    str_result = str_0
    

    if (18 > len(str_0)):
    
    
        counter_1 = 0
    
        while (counter_1 < 18 - counter_0):
            
            str_result = "0" + str_result
            
            counter_1 += 1
        
    
    
    return str_result





def open_popup_terminal(command):
    
    
    system = platform.system()

    if system == "Windows":

        subprocess.run(["cmd", "/c", f"{command}"])

    elif system == "Linux":

        subprocess.run(["gnome-terminal", "--", "bash", "-c", f"{command}; exit"])

    elif system == "Darwin":

        subprocess.run(["osascript", "-e", f'tell app "Terminal" to do script "{command}; exit"'])






def reader_of_possibility_0():



    list_result = ["0"]


    
    folder_0 = os.path.join(cwd, "space_for_mix")

    list_of_file = []

    for root, dir_, list_of_file in os.walk(folder_0):
    
        break
    

    counter_0 = 0
    
    
    while (counter_0 < len(list_of_file)):
        
        
        str_number_0 = int_to_str_0(number_0=counter_0)
        
        file_0 = os.path.join(folder_0, f"file_part_{str_number_0}.mixer")
        
        with open(file_0, "r") as f_:
        
            content = f_.read(os.path.getsize(file_0))
        
        
        list_result.append(content)
    
    
        counter_0 += 1
    
    
    
    list_result.append("0")
    
    


    return list_result





def next_step_0():

    
    
    file_1 = os.path.join(cwd, "i_run_mixer_1.txt")
    

    with open(file_1, "w", encoding="utf-8") as f_:

        f_.write("true")



    wait_for_the_next_step_0()
    




def next_step_1():

    
    
    file_1 = os.path.join(cwd, "i_run_mixer_2.txt")
    

    with open(file_1, "w", encoding="utf-8") as f_:

        f_.write("true")



    wait_for_the_next_step_1()
    




def refresher_0():
    
        
    os.system("python3 refresher_0.py")
    
    
    
    
    list_of_file = []
    
    
    counter_0 = 0
    
    
    while (counter_0 < 1):
    
    
    
        str_number_0 = int_to_str_0(number_0=counter_0)
    
        file_0 = os.path.join(cwd, "space_for_mix", f"file_part_{str_number_0}.mixer")
    
    
        with open(file_0, "w") as f_:
    
            f_.write("0")
    
        counter_0 += 1
    




def extract_and_calculate_0(list_of_result, amount):


        
    list_of_unity = []
    
    counter_0 = 0
    
    
    while (counter_0 < len(list_of_result)):
    
        #print(f"\ni_hello . int(list_of_result[counter_0]) = {int(list_of_result[counter_0])} .\n")
    
        list_of_unity.append(supported_currencies[int(list_of_result[counter_0])])
        
        counter_0 += 1
    
    
    
    result_1 = transform_and_calculate_0(list_0=list_of_unity, amount=amount)
    
    return [result_1, list_of_unity]








'''


this is the same as i_calculater_of_way_to_money_0.py but it use my math : i_math_i_am_i_0.py

so you should specify number_of_digit_after_the_floating_point

right here there is : number_of_digit_after_the_floating_point = 5



this list supported_currencies should contain all the unity of money that are transformable .



like for example right here i have 3 unity that can be transformed :


"DZD", "EUR", "USD"



and now this list list_of_result should contain all the possibility of combination between 2 unity . it mean like that :





list_of_result = [

                ['DZD', "0.9", 'DZD'], ['DZD', "0.01", 'EUR'], ['DZD', "0.01", 'USD'], 
                
                ['EUR', "135.73", 'DZD'], ['EUR', "0.9", 'EUR'], ['EUR', "1.04", 'USD'], 
                
                ['USD', "116.94", 'DZD'], ['USD', "0.78", 'EUR'], ['USD', "0.9", 'USD']
                
                ]


this include what the profit that the transformer get from the transformation .


and then you should enter your first amount that you want to start with right here :


amount = "5.0"

there is 1 condition :

    if you prefer a unity . you should start with right here :
    
        supported_currencies 
        
    it mean when i prefer starting with DZD i will do :
    
        
                
        
        supported_currencies = [
        
                                    "DZD", "EUR", "USD"
        
                                     ]
        
        

    it mean when i prefer starting with EUR i will do :


        supported_currencies = [
        
                                    "EUR", "DZD", "USD"
        
                                     ]
        
    
    the unity that you start with will be at the end also .
    
    the result will be something like that oif you prefer DZD :
    
         max_0 = ["16.9662500000", ['DZD', 'EUR', 'DZD', 'EUR', 'DZD', 'EUR', 'DZD', 'EUR', 'DZD']] .


    it mean from the amount that you where having wich is "5.0" . you will have "16.9662500000" after transforming like that :
    
        from 'DZD' to 'EUR' 
        
        and all what are from 'EUR' to 'DZD'
        
        and all what are from 'DZD' to 'EUR'
         
        and all what are from 'EUR' to 'DZD'
          
        and all what are from 'DZD' to 'EUR'
           
        and all what are from 'EUR' to 'DZD'
            
        and all what are from 'DZD' to 'EUR'
             
        and all what are from 'EUR' to 'DZD'
    
    
    
    at the end you will have "16.9662500000" DZD .

    and you can repeat all the process but this time with "16.9662500000" DZD .
    
    and repeat again . and repeat again . and repeat again .
    
    until you have money . until you are satisfied .
    


    if you want to run this program you should type : 
    
        python3 i_calculater_of_way_to_money_1.py init

    if you are in the beginning .
    
    if you want to do more calcule you can increace number_of_calcule to be more .
    
    this program use my mixer to find the best solution .

        
    
    





now you should put latest type of int . for example :


    latest_type_of_int = "int64_t"



and the length of the latest number with [ my 10 ] ( 10 ** n ) . for example : 


    the_length_of_1_complete_number_of_your_int = "18"


because int64_t <==> ( 2 ** ( 64 - 1 ) ) > 10 ** 18 .

the biggest number that can be gotten from 10 to the power is : 10 ** 18 .


this 18 you should put it on here the_length_of_1_complete_number_of_your_int .



( 2 ** ( 64 - 1 ) ) ==  9 223 372 036 854 775 808

( 10 ** 18 )        ==  1 000 000 000 000 000 000


you should do the number that are with : 10 ** n . but he is less then : 2 ** ( 64 - 1 ) .


i am speaking about 64 because i have a computer with 64 bit in the processor . 

the reason that i am making ( 64 - 1 ) and not ( 64 ) is that i am using the singed int and not the unsinged . for example :
    
    'int64_t' and not 'uint_64_t'
    
    
    




    

'''








def main(argv_1):




    


    #supported_currencies = [

                                #"DZD", "EUR", "USD"

                                 #]





    #list_of_result = [

                    #['DZD', "0.9", 'DZD'], ['DZD', "0.01", 'EUR'], ['DZD', "0.01", 'USD'], 

                    #['EUR', "135.73", 'DZD'], ['EUR', "0.9", 'EUR'], ['EUR', "1.04", 'USD'], 

                    #['USD', "116.94", 'DZD'], ['USD', "0.78", 'EUR'], ['USD', "0.9", 'USD']

                    #]


      
    
    
    
    #print(f"\n\n\n list_of_result = {list_of_result} .\n\n")
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    
    if ((len(argv_1) > 0)):
    
    
        if (argv_1 == "init"):
    
            refresher_0()
    
    
    
    
        file_1 = os.path.join(cwd, "Economic_Partner_official_produced_mixer_9.c")
    
    
        with open(file_1, "r") as f_:
    
            content = f_.read(os.path.getsize(file_1))
    
    
    
    
        content = content.replace("___number_of_chunk___", number_of_chunk_0)
    
    
    
        content = content.replace("___postion_of_max_range___", str(len(supported_currencies) - 1))
        
        
                
        content = content.replace("int64_t", latest_type_of_int)
        
        
        
        content = content.replace("#define i_Number_of_digits_max 18", f"#define i_Number_of_digits_max {the_length_of_1_complete_number_of_your_int}")
        
        
    
    
        file_2 = os.path.join(cwd, "Economic_Partner_official_produced_mixer_9_0.c")
    
    
        with open(file_2, "w") as f_:
    
            f_.write(content)
    
    
    
    
        os.system("gcc Economic_Partner_official_produced_mixer_9_0.c -o E_P_o_p_mixer_9_0")
    
    
    
        open_popup_terminal(command="./E_P_o_p_mixer_9_0")
    
    
    
    
    
    
    
    
    
    
    
    print("\n\n    start of calcul :")
    
    
    
    
    
    
    t1 = time.time()
    
    
    
    
    
    amount = "1.0"
    
    max_0 = ["0.0", []]
    
    print(f"\n\n    start with amount = {amount} .\n\n")
    
    
    
    
    
    
    
    
    
    
    
    
    counter_2 = 1
    
    
    while (counter_2 <= number_of_calcule):
    
    
    
        list_of_result_0 = reader_of_possibility_0()
    
    
    
        #print(f"\n\n counter_2 = {counter_2} . list_of_result_0 = {list_of_result_0} .\n\n\n")
    
        
    
        result = extract_and_calculate_0(list_of_result=list_of_result_0, amount=amount)
    
            
        
    
        
        
        s1 = f"{result[0][1]}"
        
        s2 = f"{max_0[0]}"
        
        
        bool_0 = i_math_i_am_i_0.my_superieur_s_n_1(s1=s1, s2=s2, number_of_digit_after_the_floating_point_=number_of_digit_after_the_floating_point)
        
    
    
    
    
        if ((bool_0 == True)):
    
    
            max_0[0] = result[0][1]
    
    
            while (len(max_0[1]) > 0):
    
                max_0[1].pop(0)        
    
            counter_0 = 0
    
            while (counter_0 < len(result[1])):
    
                max_0[1].append(result[1][counter_0])
    
                counter_0 += 1
    
    
    
        next_step_0()
    
    
        counter_2 += 1
    
    
    
    
    file_0 = os.path.join(cwd, "i_run_mixer_2.txt")
    
    with open(file_0, "w") as f_:
    
        f_.write("true")
    
    
    
    
    
    
    
    
    
    
    
    
    
    t2 = time.time()
    
    
    
    print("\n\n    finish of calcul .")
    
    
    print(f"\n\n    max_0 = {max_0} .\n\n    time = {t2 - t1} second \n\n")
    
    
    
    return max_0





    
if __name__ == "__main__":
    
    
    
    main(argv_1="init")
    
    
    


























