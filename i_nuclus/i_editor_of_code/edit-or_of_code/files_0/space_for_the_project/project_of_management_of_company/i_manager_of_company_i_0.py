...
try:
    ...
    class Window(QWidget):
        def __init__(self):
            ...
            i_folder_i_0 = "i_folder_i_0"
            
            i_file_of_image_i_0 = os.path.join(cwd, i_folder_i_0, "image_of_background_1.png")
            
            
            
             # إعداد الباليت مع الصورة
            palette = QPalette()
            palette.setBrush(QPalette.Window, QBrush(QPixmap(i_file_of_image_i_0)))
            self.setPalette(palette)
    

            ## استعمال QSS لتعيين الخلفية
            #self.setStyleSheet("QMainWindow { background-image: url(" + f"{i_file_of_image_i_0}" + "); background-repeat: no-repeat; background-position: center; }")
