...
theta = np.array([0.4, 0.2, 0.1, -0.4, -0.5])
omega = np.zeros(n)

# الثوابت الفيزيائية
L = 1.0          # طول الخيط
g = 9.81         # الجاذبية
coupling = 0.5   # شدة الترابط
damping = 0.02   # الاحتكاك (إخماد التذبذبات)
dt = 0.05        # خطوة الزمن
...
def update(frame):
    ...
    # القوة الناتجة عن الرقاصات + مرونة اللوح
    force_from_pendulums = np.sum(np.sin(theta)) * 0.02
    restoring_force = -k_plate * x_plate  # قوة استرجاع نحو المركز
