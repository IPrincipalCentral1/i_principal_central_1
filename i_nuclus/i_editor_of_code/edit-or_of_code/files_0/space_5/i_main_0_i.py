...
list_of_balls = [
    ...
    ("#000000", deep_scaled_arc, "منحنى أكثر انحناءً 2"),
...
def update(frame):
    ...
    for i, ball in enumerate(balls_data):
        ...
        if t < ball["t_final"]:
            texts[i].set_text(f"{ball['label']}: t = {t:.2f} s")
        else:
            texts[i].set_text(f"{ball['label']}: t_final = {ball['t_final']:.2f} s ✅")
